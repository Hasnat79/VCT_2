...
def get_args_parser():
    ...
    # dataset parameters
    parser.add_argument('--dataset_path', type=str, default='VCT_2/data/twitter_images/dummy12123')
    parser.add_argument('--img_resolution', type=int, default=256)
    parser.add_argument('--crop_resolution', type=int, default=224)
    parser.add_argument('--test_selected_subsets', nargs='+', required=True, default='')
    ...
    parser.add_argument('--pretrained_model', type=str, default="VCT_2/fake_detection_models/fat_former/checkpoints/fatformer_4class_ckpt.pth")
...
def evaluate(model, data_loaders, device, args=None, test=False):
    ...
    for data_name, data_loader in data_loaders.items():
        ...
        test_f1.append(f1)
    output_strs = []
    for idx, [name, ap, acc, racc, facc, precision, recall, f1] in enumerate(zip(test_dataset + ["mean"], test_AP + [np.mean(test_AP)], test_ACC + [np.mean(test_ACC)], test_real_ACC + [np.mean(test_real_ACC)], test_fake_ACC + [np.mean(test_fake_ACC)], test_precision + [np.mean(test_precision)], test_recall + [np.mean(test_recall)], test_f1 + [np.mean(test_f1)])):
        output_str = "({} {:10}) acc: {:.2f}; ap: {:.2f}; racc: {:.2f}; facc: {:.2f}; precision: {:.2f}; recall: {:.2f}; f1: {:.2f};".format(idx, name, acc*100, ap*100, racc*100, facc*100, precision*100, recall*100, f1*100)
        output_strs.append(output_str)
        print(output_str)
    
    return "; ".join(output_strs), np.mean(test_AP), np.mean(test_ACC), np.mean(test_precision), np.mean(test_recall), np.mean(test_f1)
# @torch.no_grad()
# def evaluate(model, data_loaders, device, args=None, test=False):
#     model.eval()
#     if test:
...
def main(args):
    ...
    if args.eval:
        ...
        # print(f"checkpoint[['model']]: {checkpoint['model']}")
        # exit()
