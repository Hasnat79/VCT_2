...
from sklearn.metrics import accuracy_score, roc_auc_score, roc_curve, average_precision_score, recall_score
import importlib
...
def validation_cluster(model, args, test_file='', data_loader=None):
    ...
    for _ in range(10000):
        ...
        if pos == 1:
            feat1, feat2 = random.sample(label2features_dict[random.sample(list(keys), 1)[0]], 2)
        else:
            label1, label2 = random.sample(list(keys), 2)
    ...
    recall = recall_score(gt_labels_list,np.round( prob_labels_list))

    model.train()
    return recall, accuracy, ap
...
def validation_similarity(model, args, test_file='', data_loader=None):
    ...
    recall = recall_score(gt_labels_list, prob_labels_list)
...
def main(args):
    ...
    if args.test_file == '':
        test_file_list = [
            ...
            ('annotation_DALLE3/Test_num31818.txt', 'DALLE3'),
  #          ('annotation/Test_DALLE3V4_num1354.txt', 'DALLE3V4'),
   #         ('annotation/Test_DALLE3V5_num2000.txt', 'DALLE3V5'),
    ...
    for epoch in range(args.epoches):
        ...
        for test_file, nickname in test_file_list:
            recall, accuracy, ap = validation_cluster(model, args, test_file)
            # test_auc, test_acc, test_ap = validation_similarity(model, args, test_file)
            #test_score_list.append(test_auc)
            logger.info('Score of %s: recall: %5.4f, accuracy: %5.4f, ap: %5.4f' % (nickname, recall, accuracy, ap))
...
if __name__ == '__main__':
    ...
    conf.add_argument("--data_root", type=str, default='/path/to/your/dataset/Twitter/Twitter_test/',
                      help="The root folder of training set.")
    conf.add_argument("--train_file", type=str,
                      default='/path/to/your/dataset/LASTED/annotation_DALLE3/Test_num31818.txt')
    conf.add_argument("--test_file", type=str,
                      default='/path/to/your/dataset/annotation_DALLE3/Test_num31818.txt')
    conf.add_argument('--val_ratio', type=float, default=0.005)
    conf.add_argument('--isTrain', type=int, default=0)
